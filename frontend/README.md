...
- Google Identity Services for authentication (ID token, no client secret)
...
### Foundation
- **Authentication** — Google Identity Services sign-in button with cached-token restore on reload
...
├── hooks/
│   ├── useAuth.ts               # Current ID token + auth status
│   ├── useGoogleSignIn.ts       # Google Identity Services lifecycle
│   └── useAppState.ts           # State access hook
├── reducers/
│   └── appReducer.ts            # State transitions
├── config/
│   └── authConfig.ts            # Google OAuth configuration
...
Google sign-in uses the [Google Identity Services](https://developers.google.com/identity/gsi/web) script loaded in `index.html` — no npm dependency required.
