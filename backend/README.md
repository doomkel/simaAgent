...
- Authenticates requests via JWT Bearer tokens issued by Google (validated against Google's OIDC discovery/JWKS)
- Communicates with Foundry Agent Service using a user-assigned managed identity
- Streams AI agent responses via Server-Sent Events (SSE)

## Key Features

- **Single Container**: Serves both API and frontend from `wwwroot`
- **JWT Authentication**: Validates Google ID tokens (signature, audience, and `hd` Workspace domain claim)
...
- **Environment-Aware Auth**: ChainedTokenCredential (dev) / User-assigned ManagedIdentity (prod) for Foundry calls
...
| Azure.Identity | ManagedIdentityCredential + ChainedTokenCredential |
| Microsoft.AspNetCore.Authentication.JwtBearer | Validates Google-issued ID tokens via OIDC discovery |
