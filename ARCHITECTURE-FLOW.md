...
stateDiagram-v2
    ...
    state Production {
        [*] --> MIOnly
        MIOnly --> CredentialReady: User-assigned ManagedIdentityCredential
    }
...
|-------|-------------|-------------|
| `initializing` | App startup, checking cached Google ID token | `null` |
...
|-----|--------|---------|---------|
| `GOOGLE_CLIENT_ID` | .env | Google OAuth 2.0 Client ID (validates ID token audience) | `123-abc.apps.googleusercontent.com` |
| `GOOGLE_HOSTED_DOMAIN` | .env | Workspace domain enforced via the ID token `hd` claim | `3styk.com` |
| `AI_AGENT_ENDPOINT` | .env | AI Foundry project URL | `https://....api.azureml.ms` |
| `AI_AGENT_ID` | .env | Agent name (v2 API) | `my-agent` |
| `ASPNETCORE_ENVIRONMENT` | Environment | Development/Production | `Development` |
| `MANAGED_IDENTITY_CLIENT_ID` | Container App env | User-assigned MI client ID for Foundry calls | `abc123-...` |
...
The `.env` file is auto-generated by `postprovision.ps1` during `azd up` (after Bicep provisions infrastructure).
...
| [frontend/src/contexts/AppContext.tsx](frontend/src/contexts/AppContext.tsx) | Provider with centralized auth/chat state |
